...
for (var i = 0; i < loadedDrives.length; i++) {
		if (loadedDrives[i].name == partitionName) {
			...
			//win.openApp("mean.files.app",loadedDrives[i].mount);
...
/*$("#drives").append('<a href="javascript:void(0);" class="shortcut iconSizeA"> <img src="icons/drive-removable-media-usb.svg"> <small> don </small></a>');*/
...
function loadUsages(callback, callbackVariableA) {
  ...
  var njds = require('/usr/meanOs/systemX/Shared/node_modules_fixed/nodejs-disks/lib/disks.js');
    njds.drives(
        function (err, drives) {
            njds.drivesDetail(
                drives,
...
function loadFilesApp(location) {
	win.openApp("mean.files.app",location);
}
...
for (var i = 0; i < loadedDrives.length; i++) {
		if (loadedDrives[i].name == partitionName) {
			//console.log("opening mean.files.app with",loadedDrives[i]);
			win.openApp("mean.files.app",loadedDrives[i].mount);
			setTimeout(function(){ doNotCloseStack = false; closeMenu(); }, 2000);
			break;
		}
...
if (data[i].label != "meanOs alpha" && data[i].fstype != "" && data[i].uuid != "" && data[i].fstype !="swap" && data[i].type !="disk") {
...
$("#mainBg").append('<div id="mountedContextMenu" class="contextMenu dropdown open hidden" style="margin-left: 180px;">'
                        +'<ul class="dropdown-menu dropdown-menu-alt zeroHeight" role="menu">'
                            +'<li id="openFilesOption" role="presentation" class="driveMenuItem"><a id="filesOption" role="menuitem" tabindex="-1" href="javascript:void(0);"><span><img src="../apps/mean.files.app/icon.svg"></span>Open in Files</a></li>'
...
var stackIconTemplate = '<li><a id="drivesStack" stackID=0 href="javascript:void(0);" class="shortcut deskIcon stackIcon" title="Drives" folder-control="folderContents">'
				...
				+'<figure class="stack '+stackStyle+' notActive">'
					+'<img src="../../mean.explorebar/icons/drive-harddisk.svg" alt="img01"/>'
					+'<img src="../../mean.explorebar/icons/drive-harddisk-system.svg" alt="img02"/>'
					+'<img src="../../mean.explorebar/icons/drive-harddisk-system_mount.svg" alt="img03"/>'
				+'</figure>'
...
win.openApp("mean.files.app",fileToOpen.location);
setTimeout(function(){ doNotCloseStack = false; closeMenu(); }, 2000);
} else {
...
for (var i = 0; i < stackContents.length; i++) {
	if (stackContents[i].isDirectory) {
		var fileIcon = "../../apps/mean.files.app/icons/"+win.resolveFileType("*folder*",true);
		var fileType = "folder";
	} else {
		var fileIcon = "../../apps/mean.files.app/icons/"+win.resolveFileType(stackContents[i].file.split('.').pop(),true);
